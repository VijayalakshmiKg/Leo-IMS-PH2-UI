...
export class LocationHomeComponent implements OnInit {
  ...
  constructor(public route: Router, public locationServ: LocationService, public utilSer: UtilityService) {
  ...
  ngOnInit(): void {
    ...
    for (var i = 0; i < childmenu.subMenus.length; i++) {
      if (childmenu.subMenus[i].subMenuName.toLowerCase() == 'locations') {
        this.permissions = childmenu.subMenus[i];
        this.locationServ.permissions = this.permissions;
      }
    }
  ...
  sortByValue(event: any) {
    this.sortCode = event;
    this.locationServ.getSearchAndSortBy("", this.sortCode, this.statusFilter, this.pageSize, this.pageNumber).then((res: any) => {
  ...
  searchLocation(event: any) {
    ...
    if (event.target.value.length > 1) {
      this.locationServ.getSearchAndSortBy(event.target.value, this.sortCode, this.statusFilter, this.pageSize, this.pageNumber).then((res: any) => {
        this.totalLocation = res.totalCount;
        this.totalPages = res.totalCount;
        this.locationList = res.data;
      }).catch((err: any) => {
  ...
  addLocation() {
    this.locationServ.viewLocationIndex = 0;
    this.locationServ.editLocationRecord = null
    this.route.navigateByUrl('/home/location/addLocation')
  }

  getAllLocationList() {
    this.locationServ.getAllLocations().then((res: any) => {
      this.totalLocation = res.totalCount;
      this.totalPages = res.totalCount;
      this.locationList = res.data;
    })
  ...
  viewLocation(location: any, index: any) {
    if (this.permissions?.viewAccess) {
      this.locationServ.selectedLocationRecord = location;
      this.locationServ.viewLocationId = location;
      this.locationServ.viewLocationIndex = index;
      this.route.navigateByUrl('/home/location/viewLocation')
    }
  }
}
